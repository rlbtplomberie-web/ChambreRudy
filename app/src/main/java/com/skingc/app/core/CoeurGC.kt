package com.skingc.app.core

import android.content.Context
import com.skingc.app.CodeTriche
import java.io.File

/**
 * Facade Kotlin du coeur Dolphin.
 *
 ...
 */
class CoeurGC(ctx: Context) {

    companion object {
        init { System.loadLibrary("skingc") }
        /** Fichiers systeme reconnus par Dolphin. */
         ...
         * Resolutions internes acceptees par le coeur, mot pour mot.
         *
         * On s'arrete a 1920 x 1440 : au-dela, la relecture de l'image
         * demanderait plus d'un gigaoctet par seconde, ce qu'aucun telephone
         * ne suit.
         */
        /**
         * Definitions internes acceptees par le coeur, mot pour mot, et leur
         * nom courant. La console dessine en 320 x 240 : chaque cran multiplie
         * cette definition, ce qui reduit la pixelisation sans jamais changer
         * la taille de l'image affichee.
         */
        val RESOLUTIONS = listOf("x1 (640 x 528)", "x2 (1280 x 1056)",
                                 "x3 (1920 x 1584)", "x4 (2560 x 2112)",
                                 "x5 (3200 x 2640)", "x6 (3840 x 3168)")
        val LIBELLES = listOf("Natif", "Natif ×2", "Natif ×3",
                              "Natif ×4", "Natif ×5", "Natif ×6")
    }
    ...
    private external fun natNoires(): Int
    ...
    private external fun natTriche(index: Int, actif: Boolean, code: String)
    ...
    /** La GameCube n'a pas de BIOS : le dossier ne sert qu'au catalogue
     ...
     *
     * Dimensionne pour la plus haute resolution proposee, 1920 x 1440 : un
     * tableau trop petit faisait echouer la relecture en silence, et l'ecran
     * restait noir des qu'on montait au-dela de 1280 x 1024.
     */
    val pixels = IntArray(1920 * 1440)
    var imageL = 0
        private set
    var imageH = 0
        private set
    ...
    fun chargerJeu(f: File): Boolean {
        ...
        if (!natCharger(f.absolutePath)) {
            derniereErreur = "Jeu refusé par Dolphin (${f.name})"; return false
        }
        romChargee = true
     ...
     * Resolution interne.
     *
     * Elle multiplie la definition interne du rendu. Une valeur elevee
    ...
    fun largeurEtendue(actif: Boolean) {
        natVariable("dolphin_widescreen", if (actif) "enabled" else "disabled")
    }

    // ---------- triche ----------
    /**
     * Les codes GameCube sont au format GameShark : c'est le coeur qui les
     * interprete. On lui remet la liste complete a chaque changement, en ne
     * lui numerotant que ceux qui sont actifs.
     */
    ...
    fun tricheAppliquer(codes: List<CodeTriche>) {
        natTricheVider()
        var i = 0
        for (c in codes) if (c.actif) { natTriche(i, true, c.code); i++ }
    }
     ...
     *
     * Le jeu reste calcule en haute finesse ; c'est la carte graphique qui
     * reduit, en moyennant les pixels. Ce moyennage est ce qui adoucit
     * reellement les contours.
     */
    fun reduction(largeur: Int) = natReduction(largeur)

    /** Images entierement noires d'affilee : au-dela, la finesse ne passe pas. */
    val imagesNoires: Int get() = natNoires()
...
/**
 * Bits de la manette GameCube, dans les identifiants libretro.
 *
 * Les quatre boutons C se presentent au coeur comme le second stick : c'est la
 * convention de Dolphin, et elle permet de les jouer aussi bien au doigt
 * qu'au stick droit d'une manette physique.
 */
object Pad {
    /*
     * Manette GameCube, dans les identifiants libretro.
     *
     * Le coeur les lit comme une manette standard : A et B en bas, X et Y en
     * haut, Z sur la gachette droite du fond.
     */
    const val B = 1 shl 0
    const val Y = 1 shl 1
    ...
    /** La gachette Z, sous la manette. */
    const val Z = 1 shl 12
}
