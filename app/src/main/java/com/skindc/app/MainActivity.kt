...
import com.skindc.app.core.CoeurDc
import java.io.File
...
class MainActivity : ComponentActivity() {
    ...
    private fun installerBiosEmbarque(): String {
        ...
        try {
            for (nom in assets.list("dc/bios") ?: emptyArray()) {
                ...
                for (dossier in listOf(coeur.dossierBios, coeur.dossierSysteme)) {
                    ...
                    assets.open("dc/bios/$nom").use { e -> cible.outputStream().use { e.copyTo(it) } }
    ...
    override fun onCreate(s: Bundle?) {
        ...
        noter("assets/bios : " + (assets.list("dc/bios")?.joinToString(", ") ?: "(illisible)"))
