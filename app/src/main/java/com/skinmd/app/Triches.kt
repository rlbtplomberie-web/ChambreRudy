 ...
 *
 * Genesis Plus GX reconnait lui-meme le format de chaque code.
...
object Triches {
    ...
    private const val PREFS = "skinmd_triches"
    ...
    fun forme(code: String): String? {
        ...
        val blocs = c.split("+")
        for (b in blocs) {
            /* Game Genie : quatre caracteres, un tiret, quatre. */
            if (Regex("^[0-9A-Z]{4}-[0-9A-Z]{4}$").matches(b)) continue
            /* Code brut : six chiffres hexadecimaux, deux points, la valeur. */
            if (Regex("^[0-9A-F]{6}:[0-9A-F]{2,4}$").matches(b)) continue
            return null
        }
     ...
     *
     * Elle couvre 1042 jeux de Mega Drive et plus de trente-cinq mille codes,
     * tires des fichiers .cht de libretro. On ne la lit qu'une fois, et
     * seulement si l'on en a besoin.
     */
    private fun base(ctx: Context): JSONObject {
        base?.let { return it }
        val o = try {
            val texte = ctx.assets.open("triches_md.json")
