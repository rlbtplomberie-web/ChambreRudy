...
import com.rudy.chambre.core.CoeurPlus as Coeur
 ...
 * L'application.
 *
 * Elle assemble le coeur Genesis Plus GX et l'habillage qui dessine la console.
...
class MdActivity : ComponentActivity() {
    ...
    private val prefs by lazy {
        getSharedPreferences("skinmd", Context.MODE_PRIVATE)
    }
    ...
    /*
     * START a maintenant sa pastille sur les trois habillages. Reste MODE,
     * qui ne sert qu'a basculer une manette six boutons en trois boutons pour
     * les rares jeux qui s'y perdent : il n'appelait pas sa propre touche, on
     * l'envoie depuis la barre de menu sous forme d'impulsion — tenue
     * quelques images, puis relachee toute seule. Six images suffisent : un
     * jeu qui scrute sa manette une fois par image ne peut pas la manquer, et
     * c'est trop court pour qu'une pression compte double.
     */
    @Volatile private var impulsion = 0
    @Volatile private var impulsionRestante = 0

    private fun envoyer(bit: Int, nom: String) {
        impulsion = bit
        impulsionRestante = 6
        Toast.makeText(this, nom + " envoyé", Toast.LENGTH_SHORT).show()
    }
    ...
    private fun basculerModifier() {
        ...
        if (vue.modifier) {
            Toast.makeText(this,
                "Touche une pièce, puis tire ses poignées : les côtés " +
                "étirent en largeur ou en hauteur, les coins en diagonale",
    ...
    private fun libelleRapport() =
        "Image : " + if (vue.respecterRapport) "4:3" else "plein cadre"
    ...
    private fun construirePanneauModif(): LinearLayout {
        ...
        rangee.addView(bouton(libelleRapport()) {
            vue.respecterRapport = !vue.respecterRapport
            prefs.edit().putBoolean("rapport43", vue.respecterRapport).apply()
            (rangee.getChildAt(2) as Button).text = libelleRapport()
            vue.invalidate()
            Toast.makeText(this,
                if (vue.respecterRapport) "L'image garde ses proportions dans le cadre"
                else "L'image épouse exactement le cadre",
        ...
        ligneOpacite.addView(SeekBar(this).apply {
            ...
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                ...
                override fun onStopTrackingTouch(sb: SeekBar) {
                    Dispositions.poserOpacite(this@MdActivity, vue.opaciteBoutons)
                }
            })
    ...
    private fun extraire(uri: Uri, nom: String): File? {
        ...
        if (sorties.isEmpty() && entrees > 0)
            throw IllegalStateException(
                "Cette archive ne contient aucun jeu Mega Drive " +
                "($entrees fichier${if (entrees > 1) "s" else ""} dedans)")
        return sorties.firstOrNull()
    }

    private fun appliquerReglages() {
        /* La GBA n'a pas de palette a choisir : seule la correction
           colorimetrique se regle, et elle est facultative. */
        vue.respecterRapport = prefs.getBoolean("rapport43", false)
        coeur.correctionCouleur(prefs.getBoolean("correction", true))
        coeur.sauteImages(prefs.getBoolean("frameskip", false))
    }
        ...
        fil = Thread {
            ...
            while (tourne) {
                ...
                for (i in 0 until tours) {
                    val renfort = if (impulsionRestante > 0) {
                        impulsionRestante--; impulsion
                    } else { impulsion = 0; 0 }
                    coeur.image(boutons or renfort)
        ...
        for (c in codes) {
            ll.addView(CheckBox(this).apply {
                ...
                setOnCheckedChangeListener { _, v ->
                    c.actif = v
                    Triches.enregistrer(this@MdActivity, jeu, codes)
                }
                setOnLongClickListener {
                    android.app.AlertDialog.Builder(this@MdActivity)
    ...
    private fun ouvrirReglages() {
        ...
        /* Les palettes de la Game Boy n'ont plus lieu d'etre : la Game Boy
           Advance affiche ses propres couleurs. Reste la correction, qui
           rattrape l'ecran tres delave de la console d'origine. */
        ll.addView(titre("Couleurs"))
        ll.addView(CheckBox(this).apply {
            text = "Correction colorimétrique"
            isChecked = prefs.getBoolean("correction", true)
            setOnCheckedChangeListener { _, v ->
                prefs.edit().putBoolean("correction", v).apply()
                appliquerReglages()
            }
        })
        ll.addView(titre("Image"))
        ll.addView(CheckBox(this).apply {
            text = "Garder le 4:3 de la console"
            isChecked = prefs.getBoolean("rapport43", false)
            setOnCheckedChangeListener { _, v ->
                prefs.edit().putBoolean("rapport43", v).apply()
                appliquerReglages()
                vue.invalidate()
                Toast.makeText(this@MdActivity,
                    if (v) "L'image garde ses proportions dans le cadre"
                    else "L'image épouse exactement le cadre",
                    Toast.LENGTH_SHORT).show()
            }
        })
        ll.addView(CheckBox(this).apply {
            text = "Sauter des images si ça rame"
            isChecked = prefs.getBoolean("frameskip", false)
            setOnCheckedChangeListener { _, v ->
                prefs.edit().putBoolean("frameskip", v).apply()
                appliquerReglages()
            }
        })
    ...
    override fun onCreate(s: Bundle?) {
        ...
        coeur = Coeur(this, "libgenesis.so")
        if (!coeur.pret) noter("cœur non prêt : " + (coeur.derniereErreur ?: "?"))

        /* Mise a jour : l'image epouse desormais le cadre qu'on lui donne.
           On efface une bonne fois le format retenu par l'ancienne version,
           sinon un reglage garde depuis des mois continuerait a imposer ses
           bandes noires. Le choix reste possible ensuite. */
        if (!prefs.getBoolean("format_libre", false)) {
            prefs.edit().putBoolean("format_libre", true).putBoolean("rapport43", false).apply()
        }
        ...
        barre = LinearLayout(this).apply {
            ...
            addView(bouton("MODE") { envoyer(Pad.MODE, "MODE"); basculerBarre() })
            ...
            addView(bouton("Défaut") {
                android.app.AlertDialog.Builder(this@MdActivity)
                    ...
                    .setPositiveButton("Rétablir") { _, _ ->
                        ...
                        Toast.makeText(this@MdActivity, "Rétabli",
                                       Toast.LENGTH_SHORT).show()
                    }
                    .setNegativeButton("Annuler", null)
                    .show()
