...
class N3dsActivity : ComponentActivity() {
    ...
    private val ESSAIS = listOf(
        ...
        Triple("citra_texture_filter", "Filtrage des textures",
               listOf("Anime4K", "Bicubic")),
        Triple("citra_use_hw_shader", "Shaders matériels", listOf("enabled", "disabled")),
        Triple("citra_use_shader_jit", "Compilation des shaders", listOf("enabled", "disabled")),
        Triple("citra_is_new_3ds", "Modèle de console", listOf("New 3DS", "Old 3DS")),
        Triple("citra_use_virtual_sd", "Carte SD virtuelle", listOf("enabled", "disabled")))
        ...
        Thread {
            ...
            try {
                ...
                val ext = nom.substringAfterLast('.', "").lowercase()
                    ...
                    if (cible == null)
                        erreur = "Aucune ROM Nintendo 3DS dans cette archive (.3ds ou .cia attendu)"
                } else {
    ...
    private fun changerPresentation() {
        ...
        val h = com.rudy.chambre.n3dsui.Dispositions.habillage(true, vue.rangPaysage)
    ...
    private fun ouvrirCatalogue() {
        ...
        Thread {
            ...
            runOnUiThread {
                if (isFinishing || isDestroyed) return@runOnUiThread
                if (roms.isEmpty()) {
                    Toast.makeText(this, "Aucun jeu Nintendo 3DS dans ce dossier", Toast.LENGTH_LONG).show()
    ...
    private fun appliquerQualite(n: Int) {
        val nb = com.rudy.chambre.n3dsui.core.Coeur3DS.RESOLUTIONS.size
        ...
        noter("résolution interne demandée : " +
              com.rudy.chambre.n3dsui.core.Coeur3DS.LIBELLES[qualite - 1])
        bQualite.text = com.rudy.chambre.n3dsui.core.Coeur3DS.LIBELLES[qualite - 1]
    }
     ...
     *
     * Citra ne lit cette option qu'au chargement. On sauvegarde donc la
     * partie, on relance, on la restaure : la finesse change sans qu'on
     * reparte du debut.
     */
    private fun relancerPourQualite() {
        val f = jeuCourant
        val res = com.rudy.chambre.n3dsui.core.Coeur3DS.LIBELLES[qualite - 1]
    ...
    private fun expliquerQualite() {
        android.app.AlertDialog.Builder(this)
            ...
            .setMessage(
                "L'écran du haut de la console est dessiné en 400 × 240 points, " +
                "celui du bas en 320 × 240. Ce réglage multiplie cette définition : " +
                "à ×4, l'écran du haut est calculé en 1600 × 960.\n\n" +
                ...
                "Le changement relance le jeu, Citra ne lisant ce réglage qu'au " +
    ...
    private fun expliquerCadence() {
        android.app.AlertDialog.Builder(this)
            ...
            .setMessage("Une console a une cadence fixe : la Nintendo 3DS produit " +
    ...
    /**
     * Citra n'a besoin d'aucun fichier prealable : il embarque son propre
     * micrologiciel. Rien a installer, contrairement aux consoles precedentes.
     */
    /**
     * Installe les donnees systeme de la 3DS.
     *
     * Citra a besoin de la police partagee — cbf_std.bcfnt — des cles aes et
     * de l'arborescence NAND de la console. Elles viennent de l'application de
     * reference, et sans elles le coeur ne peut pas demarrer un jeu.
     */
    private fun installerReglages(): String {
        ...
        return try {
            ...
            copier("skins/3ds/sysdata", File(racine, "sysdata"))
            copier("skins/3ds/nand", File(racine, "nand"))
            temoin.writeText("ok")
            noter("données système installées : " + n + " fichiers")
            "données système installées (" + n + ")"
        } catch (e: Exception) {
            noter("ECHEC de l'installation des données système : " + e.message)
            "installation impossible : " + e.message
        }
    }
    private fun installerRapport() {
        val avant = Thread.getDefaultUncaughtExceptionHandler()
        Thread.setDefaultUncaughtExceptionHandler { fil, e ->
            try {
                val texte = e.toString() + "\n" +
                    e.stackTrace.take(8).joinToString("\n") { it.toString() }
                getSharedPreferences("skin_3ds", MODE_PRIVATE).edit()
                    .putString("derniere_chute", texte).apply()
            } catch (_: Throwable) {}
    ...
    /**
     * Note l'etape en cours du demarrage, AVANT de la tenter.
     *
     * Un plantage natif ne laisse aucune trace : l'application disparait sans
     * un mot. En ecrivant l'etape avant de l'executer, la relance suivante
     * peut dire laquelle n'est jamais revenue.
     */
    private fun etape(nom: String) {
        // Ecriture SYNCHRONE : les preferences ecrivent en differe, et un
        // plantage natif survenant juste apres perdait l'information.
        try {
            val f = File(filesDir, "etape_demarrage.txt")
            java.io.FileOutputStream(f).use { flux ->
                flux.write(nom.toByteArray())
                flux.flush()
                flux.fd.sync()
            }
        } catch (_: Throwable) {}
    ...
    private fun derniereEtapeLue(): String? = try {
        val f = File(filesDir, "etape_demarrage.txt")
    ...
    override fun onCreate(s: Bundle?) {
        ...
        installerRapport()
        super.onCreate(s)
        val derniereEtape = derniereEtapeLue()
        try {
                    bordABord()

            etape("ouverture du cœur")
            coeur = Coeur3DS(this)
            installerCapteur()
            ...
            etape("création de la couche OpenGL")
            gl = VueGL(this, coeur)
            vue = SkinView(this)
            ...
            // Si l'application s'est arretee au lancement precedent, on montre ce
            // qui s'est passe : elle disparaissait sans un mot.
            prefs.getString("derniere_chute", null)?.let { chute ->
                prefs.edit().remove("derniere_chute").apply()
                noter("chute précédente : " + chute.lineSequence().first())
                android.app.AlertDialog.Builder(this)
                    .setTitle("L'application s'était arrêtée")
                    .setMessage(chute)
                    .setPositiveButton("Compris", null)
                    .show()
            }
            ...
            if (!coeur.pret) {
                android.app.AlertDialog.Builder(this)
                    ...
                    .setMessage(coeur.derniereErreur + "\n\nLe fichier libcitra.so doit être dans l'APK.")
                    .setPositiveButton("OK", null).show()
            }
            noter(bilanBios)
            etape("terminé")
            if (derniereEtape != null && derniereEtape != "terminé") {
                noter("le démarrage précédent s'est arrêté à : " + derniereEtape)
                android.app.AlertDialog.Builder(this)
                    .setTitle("Le démarrage précédent avait échoué")
                    .setMessage("Il s'est arrêté à l'étape : " + derniereEtape)
                    .setPositiveButton("Compris", null)
                    .show()
            }
        } catch (e: Throwable) {
