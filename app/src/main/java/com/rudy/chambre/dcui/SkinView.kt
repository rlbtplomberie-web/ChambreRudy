...
import com.rudy.chambre.dcui.core.Pad
...
class SkinView(ctx: Context) : View(ctx) {
    ...
    /** 0 ajuste au format d'origine, 1 remplit le cadre, 2 remplit en rognant. */
    var modeEcran = 1
        set(v) { field = v; invalidate() }
    var zoomEcran = 1.0f
    ...
    fun poserImage(pixels: IntArray, l: Int, h: Int) {
        if (l <= 0 || h <= 0 || l > 1280 || h > 1024) return
     ...
     * disposition sur place : passer par requestLayout ne suffit pas quand la
    ...
    private fun chargerSkin(dossier: String, cible: HashMap<String, Touche>): Bitmap {
        val texte = context.assets.open("skins/dc/skin/$dossier/positions.json").bufferedReader().use { it.readText() }
    ...
    private fun boucle() {
        Choreographer.getInstance().postFrameCallback(object : Choreographer.FrameCallback {
            override fun doFrame(ns: Long) {
                ...
                surCommandes?.invoke(boutons, st.x, st.y,
                    if (actifs.containsKey(Ids.L)) 1f else 0f,
                    if (actifs.containsKey(Ids.R)) 1f else 0f,
    ...
    override fun onDraw(c: Canvas) {
        ...
        // horizontales que le bouton manette fait alterner. Le fond DOIT
        // suivre la variante comme les touches, sinon on affiche l'ancienne
        // coque avec les nouvelles touches.
    ...
    private fun dessinerJeu(c: Canvas, r: Rect4) {
        ...
        val ratio = 4f / 3f
        var w = tmp.width(); var h = tmp.height()
        when (modeEcran) {
            0 -> { h = w / ratio; if (h > tmp.height()) { h = tmp.height(); w = h * ratio } }
            2 -> { h = w / ratio; if (h < tmp.height()) { h = tmp.height(); w = h * ratio } }
        }
    ...
    private val ordreCapture = Ids.FONCTIONS + listOf(Ids.FF, Ids.L, Ids.R, Ids.START,
        Ids.A, Ids.B, Ids.X, Ids.Y, Ids.CROIX, Ids.STICK)
    ...
    private val TABLE = mapOf(
        ...
        Ids.L to Pad.L, Ids.R to Pad.R, Ids.START to Pad.START)
