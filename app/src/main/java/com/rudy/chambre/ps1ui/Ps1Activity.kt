...
class Ps1Activity : ComponentActivity() {
    ...
    private fun installerBiosEmbarque() {
        try {
            for (nom in assets.list("skins/ps1/bios") ?: emptyArray()) {
                val cible = File(coeur.dossierSysteme, nom.lowercase())
                if (cible.exists()) continue
                assets.open("skins/ps1/bios/$nom").use { e -> cible.outputStream().use { e.copyTo(it) } }
            }
        } catch (_: Exception) {}
    }
        ...
        ll.addView(SeekBar(this).apply {
            ...
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                ...
                override fun onStopTrackingTouch(sb: SeekBar) {
                    Dispositions.poserOpacite(this@Ps1Activity, vue.opaciteBoutons)
                }
            })
        })
        android.app.AlertDialog.Builder(this)
