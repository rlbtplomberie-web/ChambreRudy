/*
 * Pont entre l'application et le coeur PPSSPP.
 *
...
enum {
    ...
    /* Numeros exacts de libretro : 56 est le rendu prefere. J'avais mis 73,
       qui designe tout autre chose — on ecrivait donc dans une structure
       qui n'attendait pas ce qu'on y mettait. */
    ENV_GET_PREFERRED_HW_RENDER = 56,
    ENV_SET_CONTROLLER_INFO = 35, ENV_GET_USERNAME_ = 38,
    ENV_SET_HW_SHARED_CONTEXT = 44 | 0x10000,
    ENV_SET_OPTIONS_UPDATE_CB = 69,
};

enum { PIX_0RGB1555 = 0, PIX_XRGB8888 = 1, PIX_RGB565 = 2 };
/* Types de contexte materiel, dans la numerotation de libretro. */
enum { HW_CONTEXT_OPENGLES2 = 1, HW_CONTEXT_OPENGL = 2, HW_CONTEXT_OPENGLES3 = 5 };
...
/* Types de contexte graphique, dans les valeurs exactes de libretro.
   ...
   verification refusait alors le contexte que PPSSPP demandait. */
...
static void gerer_signal(int sig, siginfo_t *info, void *ctx) {
    ...
    /*
     * On laisse une marque que l'application retrouvera au prochain
     * demarrage. Le gestionnaire Java, lui, ne voit que les erreurs Java :
     * un arret natif comme celui-ci ne laissait aucune trace exploitable, et
     * l'application repartait avec les memes reglages, pour retomber
     * aussitot. Un simple fichier vide suffit ; on n'en ecrit pas plus, un
     * gestionnaire de signal devant en faire le moins possible.
     */
    if (g_dossier[0]) {
        char m[512];
        snprintf(m, sizeof m, "%s/chute_native", g_dossier);
        int fd = open(m, O_WRONLY | O_CREAT | O_TRUNC, 0600);
        if (fd >= 0) close(fd);
    }
...
/*
 * Refuser le rendu materiel.
 *
 * La PlayStation et la Super Nintendo, qui fonctionnent depuis toujours, ne
 * l'utilisent pas : elles recoivent les pixels directement du coeur. En le
 * refusant a PPSSPP, il bascule sur son calculateur logiciel et nous remet
 * ses images de la meme facon — sans fil de rendu separe, sans tampon a
 * relire, sans synchronisation a esperer.
 */
/*
 * Refus par defaut : NON.
 *
 * Refuser le contexte a PPSSPP le faisait tomber au chargement du jeu : il
 * compte dessus meme lorsqu'il calcule ses images lui-meme. On accepte donc
 * le contexte comme la Dreamcast et la 3DS, et c'est le reglage
 * « ppsspp_software_rendering » qui lui fait produire ses pixels par la
 * fonction video, celle que je sais desormais traiter.
 */
static bool g_refuser_hw = false;
 ...
 * qu'on lui a designe et nous previent par un pointeur convenu. En logiciel,
 * il nous remet les pixels ici meme — et je les jetais, ce qui donnait un
 * ecran noir alors que l'emulation tournait parfaitement.
 */
...
static void video(const void *data, unsigned w, unsigned h, size_t pitch) {
    ...
    /* conversion vers le format de l'ecran, ARGB */
...
static bool environnement(unsigned cmd, void *data) {
    switch (cmd) {
        ...
        case ENV_SET_HW_RENDER: {
            if (g_refuser_hw) {
                noter("rendu materiel REFUSE : le coeur calculera ses images "
                      "lui-meme, comme sur les autres consoles");
                g_hw_demande = false;
                return false;
            }
             ...
             * « version demandee » que PPSSPP emploie.
             *
        ...
        case ENV_GET_VARIABLE: {
            ...
            v->value = val;
            /*
             * On note une fois les reglages que le coeur reclame et que nous
             * n'avons pas. C'est ainsi qu'on aurait vu tout de suite que
             * « ppsspp_rendering_mode » n'existait pas ici, et que le vrai
             * nom du moteur graphique nous echappait.
             */
            if (!val && v->key) {
                static const char *vus[48]; static int n = 0;
                int connu = 0;
                for (int i = 0; i < n; i++)
                    if (strcmp(vus[i], v->key) == 0) { connu = 1; break; }
                if (!connu && n < 48) {
                    vus[n++] = v->key;
                    noter("le coeur reclame le reglage « %s », que nous ne posons pas", v->key);
                }
            }
            return val != NULL;
        }
        ...
        case ENV_SET_OPTIONS_UPDATE_CB:
        ...
        case ENV_SET_HW_SHARED_CONTEXT:
            /*
             * Le coeur demande un contexte graphique PARTAGE, pour dessiner
             * depuis son propre fil. On n'en fournit pas : repondre oui
             * serait un mensonge, et il dessinerait depuis un fil sans
             * contexte. On refuse donc franchement — c'est la reponse
             * honnete, et elle le fait rester sur le fil principal.
             */
            return false;

        case ENV_GET_PREFERRED_HW_RENDER:
            /*
             * PPSSPP demande quel rendu materiel la facade prefere avant de
             * choisir son moteur graphique. Sans reponse il renoncait, puis
             * demarrait quand meme son moteur OpenGL sans contexte — d'ou
             * l'acces interdit a « Starting graphics ».
             */
            *(unsigned *)data = HW_CONTEXT_OPENGLES3;
            noter("le coeur demande le rendu prefere : OpenGL ES 3");
            return true;

        default: {
            /*
             * On note les demandes qu'on ne sait pas traiter.
             *
             * Refuser en silence laissait sans reponse la question la plus
             * importante : pourquoi le coeur ne reclame-t-il pas de contexte
             * graphique ? Chaque numero n'est note qu'une fois, pour ne pas
             * noyer le journal.
             */
            static unsigned vus[64]; static int n_vus = 0;
            unsigned c = cmd & 0xFFFF;
            for (int i = 0; i < n_vus; i++) if (vus[i] == c) return false;
            if (n_vus < 64) { vus[n_vus++] = c;
                noter("demande %u du coeur : non traitee", c); }
            return false;
        }
    }
}
...
Java_com_skinpsp_app_core_CoeurPSP_natInit(JNIEnv *env, jobject self, jstring chemin,
    ...
    /* PPSSPP cree des fichiers temporaires : sur Android il n'y a ni /tmp ni
    ...
    /*
     * dlopen retrouve ici la bibliotheque qu'Android vient de charger par
     * System.loadLibrary : meme fichier, meme instance, et son JNI_OnLoad a
     * donc bien ete execute. On ne s'en sert que pour prendre les fonctions
     * libretro.
     */
    ...
    /*
     * Les reglages de PPSSPP, dans les noms exacts du coeur.
     *
     ...
     */
    var_poser("ppsspp_internal_resolution", "1x");
    var_poser("ppsspp_texture_filtering", "Auto");
    var_poser("ppsspp_texture_scaling_level", "1x");
    var_poser("ppsspp_texture_scaling_type", "xbrz");
    var_poser("ppsspp_texture_anisotropic_filtering", "4x");
    var_poser("ppsspp_texture_deposterize", "enabled");
    /*
     * LE reglage decisif : quel moteur graphique le coeur emploie.
     *
     * Laisse a « auto », PPSSPP choisit Vulkan sur Android. Or nous ne
     * savons fournir qu'un contexte OpenGL : il ne reclamait donc aucun
     * contexte — d'ou la ligne « rendu materiel non demande » du journal —
     * puis demarrait quand meme son moteur a « Starting graphics », sans
     * rien dessous. C'est la l'acces interdit, et il tombait toujours au
     * meme endroit, quels que soient le jeu et les autres reglages.
     *
     * On impose donc OpenGL, le seul que ce pont sache servir.
     */
    var_poser("ppsspp_backend", "opengl");

    /*
     * « ppsspp_rendering_mode » n'existe pas dans ce coeur.
     *
     * Je le posais depuis le debut : le coeur l'ignorait en silence, et je
     * croyais regler quelque chose. Le vrai nom est
     * « ppsspp_skip_buffer_effects », et son sens est inverse — on le laisse
     * donc a « disabled » pour garder les effets de tampon.
     */
    var_poser("ppsspp_skip_buffer_effects", "disabled");

    /*
     * Rendu logiciel : ACTIVE.
     *
     * Le journal est formel : « rendu materiel non demande ». Le coeur ne
     * reclame jamais de contexte graphique, puis demarre quand meme son
     * moteur OpenGL au moment de « Starting graphics » — et tombe sur un
     * acces interdit, faute de contexte.
     *
     * En lui demandant explicitement son calculateur logiciel, il produit
     * ses images par la fonction video, celle qui nous en a deja livre
     * trois avant la chute. C'est plus lent qu'un rendu materiel, mais une
     * image qui s'affiche vaut mieux qu'une application qui se ferme.
     *
     * Si le contexte finit par etre reclame — la reponse au rendu prefere
     * peut y suffire — ce reglage se debranche depuis les reglages.
     */
    /*
     * Calcul logiciel ET moteur OpenGL, ENSEMBLE.
     *
     * C'est la seule combinaison que je n'avais jamais essayee : j'avais pose
     * le calcul logiciel avec le moteur laisse a « auto » — donc Vulkan, que
     * nous ne servons pas — puis le moteur OpenGL avec le calcul materiel,
     * qui reclame un contexte sur le fil du chargeur, ou il n'y en a pas.
     * Les deux tombaient a « Starting graphics ».
     *
     * Le coeur prevoit pourtant ce cas, et l'annonce dans son propre
     * journal : « Software rendering init hack for opengl triggered. » Il
     * calcule alors ses images lui-meme et n'a plus besoin d'un contexte sur
     * le fil ou il demarre son moteur graphique. Ses images arrivent par la
     * fonction video, celle qui nous en livre deja trois avant la chute.
     *
     * C'est plus lent qu'un rendu materiel. Une image lente vaut mieux
     * qu'une application qui se ferme, et on reglera la vitesse ensuite.
     */
    var_poser("ppsspp_software_rendering", "enabled");

    /*
     * On ne force plus le rendu logiciel.
     *
     * Demander a PPSSPP de calculer ses images lui-meme tout en acceptant le
     * contexte graphique le faisait tomber au chargement. On revient donc a sa
     * configuration normale, celle de la Dreamcast et de la 3DS, qui
     * fonctionnent : contexte accepte, rendu materiel, tampon relu.
     *
     * Ce qui change depuis les essais precedents, et qui explique sans doute
     * l'ecran noir : le tampon fait maintenant 1920 sur 1088 au lieu de 480
     * sur 272. Le coeur dessinait plus grand que lui.
     */
    var_poser("ppsspp_frameskip", "0");
    var_poser("ppsspp_auto_frameskip", "disabled");
    /*
     * Recompilateur : « IR JIT » plutot que « JIT ».
     *
     * Le JIT classique traduit le code de la console en code natif et l'ecrit
     * dans de la memoire executable. Android le refuse de plus en plus, et
     * l'emulation tombait apres quelques images — exactement ce qu'a montre le
     * journal. « IR JIT » passe par une representation intermediaire, sans
     * memoire executable : presque aussi rapide, et stable. Le reglage reste
     * modifiable pour retenter l'autre.
     */
    var_poser("ppsspp_cpu_core", "IR JIT");
    var_poser("ppsspp_language", "French");
    var_poser("ppsspp_button_preference", "Cross");
    /*
     * Acces memoire rapide : desactive.
     *
     * Il supprime les verifications de bornes pour gagner un peu de vitesse.
     * Un jeu qui lit hors de sa memoire provoque alors un acces interdit au
     * lieu d'une valeur nulle. C'est le second suspect de l'arret constate.
     */
    var_poser("ppsspp_fast_memory", "disabled");
     ...
     *
     * Je l'avais repousse au premier jeu par prudence. Mais c'est l'ordre
     * attendu par un coeur libretro, et c'est celui qu'emploient les
     * emulateurs qui fonctionnent. Repousser cette etape laissait le coeur
     * dans un etat incomplet : il ne prenait jamais en compte le contexte
     * graphique, et son image restait noire.
     */
    p_init();
    g_coeur_init = true;
    g_init = true;
...
JNIEXPORT jboolean JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natGlInit(JNIEnv *env, jobject self, jint w, jint h) {
    (void)env; (void)self;
    if (!creer_fbo(w, h)) return JNI_FALSE;
    /* On previent le coeur que le contexte graphique existe. Sans cette
       annonce, il n'installe jamais son rendu et son image reste noire. */
...
JNIEXPORT jboolean JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natGlTaille(JNIEnv *env, jobject self, jint w, jint h) {
    (void)env; (void)self;
...
JNIEXPORT jboolean JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natCharger(JNIEnv *env, jobject self, jstring fichier) {
    (void)self;
    ...
    {   /* on verifie que le fichier est bien lisible avant de le confier */
        FILE *t = fopen(copie, "rb");
        if (!t) { noter("REFUS : fichier illisible a ce chemin"); return JNI_FALSE; }
        fseek(t, 0, SEEK_END);
        noter("  taille lue : %ld octets", ftell(t));
        fclose(t);
    }
    noter("  tampon actuel : %d x %d, contexte %s", g_fw, g_fh,
          g_hw_pret ? "pret" : "pas encore");
    ...
    /*
     * On ne recree PAS le tampon ici.
     *
     * Je le redimensionnais a la taille annoncee par le coeur, juste apres le
     * chargement. Les emulateurs qui fonctionnent ne le font pas : ils creent
     * leur tampon une fois, a l'ouverture de la surface, et se contentent de
     * signaler au coeur que le contexte existe. Detruire puis recreer un
     * tampon sous un coeur qui vient de s'installer dessus est un risque
     * inutile — et c'est la que l'application tombait.
     */
    if (g_hw_demande && g_hw.context_reset) {
        g_etape = "installation du contexte graphique";
        noter("le coeur installe son rendu sur notre tampon %d x %d", g_fw, g_fh);
        g_hw.context_reset();
        g_hw_pret = true;
        noter("rendu installe");
    }

    g_etape = "declaration de la manette";
    p_set_controller_port_device(0, DEVICE_JOYPAD);
    g_charge = true;
    g_images = 0;
    pthread_mutex_lock(&g_son_verrou); g_son_n = 0; pthread_mutex_unlock(&g_son_verrou);
    noter("mode : rendu materiel %s, images logicielles %s",
          g_hw_demande ? "accepte" : "non demande",
          g_soft_pret ? "deja recues" : "pas encore");
...
Java_com_skinpsp_app_core_CoeurPSP_natEjecter(JNIEnv *env, jobject self) {
    (void)env; (void)self;
    if (g_charge) {
...
JNIEXPORT void JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natReset(JNIEnv *env, jobject self) {
    (void)env; (void)self;
    if (g_charge) p_reset();
}

JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natGlImage(JNIEnv *env, jobject self, jint boutons,
                                              jfloat sx, jfloat sy) {
    (void)env; (void)self;
    ...
    /*
     * On oublie l'image logicielle de l'image precedente.
     *
     * PPSSPP livre quelques images par la fonction video au demarrage — son
     * logo, ses menus — puis passe au rendu materiel pour le jeu lui-meme.
     * Or le drapeau qui signale « une image logicielle est prete » n'etait
     * jamais rabaisse. La relecture, qui le teste en premier, renvoyait donc
     * eternellement cette poignee d'images de demarrage, et le jeu ne
     * s'affichait jamais.
     *
     * En le rabaissant avant chaque tour, le drapeau ne dit plus « il y a
     * DEJA eu une image logicielle » mais « il y en a une POUR CETTE
     * IMAGE-CI », ce qui est la question posee.
     */
    g_soft_pret = false;
    ...
    p_run();
    if (g_hw_demande) {
        /* PPSSPP travaille sur son propre fil de rendu : ses commandes ne
    ...
    g_images++;
    if (g_images == 30) {
        noter("apres 30 images : tampon demande %lu fois, images logicielles %s",
              g_fb_demande, g_soft_pret ? "OUI" : "non");
        if (g_fb_demande == 0 && !g_soft_pret)
            noter("ATTENTION : le coeur ne dessine ni dans notre tampon ni en logiciel");
    }
    return 1;
}
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natGlDessiner(JNIEnv *env, jobject self,
    ...
    /* la portion de la texture reellement dessinee par le coeur */
...
JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natLirePixels(JNIEnv *env, jobject self, jintArray sortie) {
    (void)self;
    if (!g_charge) return 0;
    g_etape = "relecture de l'image";

    /* Le coeur a dessine en logiciel : ses pixels sont deja prets, il n'y a
       rien a relire de la carte graphique. */
    if (g_soft_pret) {
    ...
    /*
     * La clarte se mesure AVANT de rendre le tampon.
     *
     * Elle etait calculee apres glUnmapBuffer, donc sur une adresse que le
     * pilote venait de reprendre : valeur au hasard, et plantage possible
     * selon l'appareil. Le diagnostic lui-meme etait devenu une panne.
     */
    {
        unsigned long somme = 0;
    ...
    /* Le tampon est rendu au pilote une fois toutes les lectures faites. */
    glUnmapBuffer(GL_PIXEL_PACK_BUFFER);
    glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
...
JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natSon(JNIEnv *env, jobject self, jshortArray sortie) {
    (void)self;
    jsize cap = (*env)->GetArrayLength(env, sortie);
...
JNIEXPORT void JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natSonVider(JNIEnv *env, jobject self) {
    (void)env; (void)self;
    pthread_mutex_lock(&g_son_verrou); g_son_n = 0; pthread_mutex_unlock(&g_son_verrou);
}

JNIEXPORT void JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natVariable(JNIEnv *env, jobject self,
...
/* Choix du mode de rendu, avant le chargement du jeu. */
JNIEXPORT void JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natRenduLogiciel(JNIEnv *e, jobject s, jboolean oui) {
    (void)e; (void)s;
    g_refuser_hw = (oui == JNI_TRUE);
    noter("mode de rendu : %s", g_refuser_hw ? "logiciel" : "materiel");
}

JNIEXPORT void JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natReduction(JNIEnv *env, jobject self, jint largeur) {
    (void)env; (void)self;
    g_reduction = largeur;
...
JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natLogiciel(JNIEnv *e, jobject s) {
    (void)e; (void)s; return g_soft_pret ? 1 : 0;
}

JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natClarte(JNIEnv *e, jobject s) {
    (void)e; (void)s; return (jint)g_clarte;
}

JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natImages(JNIEnv *e, jobject s) {
    (void)e; (void)s; return (jint)(g_images & 0x7FFFFFFF);
}

JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natMaxL(JNIEnv *e, jobject s) { (void)e; (void)s; return (jint)g_maxw; }
JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natMaxH(JNIEnv *e, jobject s) { (void)e; (void)s; return (jint)g_maxh; }
JNIEXPORT jfloat JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natFps(JNIEnv *e, jobject s) { (void)e; (void)s; return (jfloat)g_fps; }
JNIEXPORT jint JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natFrequence(JNIEnv *e, jobject s) { (void)e; (void)s; return (jint)g_sample_rate; }

JNIEXPORT jbyteArray JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natSauver(JNIEnv *env, jobject self) {
    (void)self;
...
JNIEXPORT jboolean JNICALL
Java_com_skinpsp_app_core_CoeurPSP_natRestaurer(JNIEnv *env, jobject self, jbyteArray etat) {
    (void)self;
