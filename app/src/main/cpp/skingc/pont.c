/*
 * Pont entre l'application et le coeur Dolphin (format libretro).
 *
 * Difference majeure avec les ponts NES, Super Nintendo et PlayStation :
 * Dolphin dessine en OpenGL dans un tampon que
...
enum {
    ...
    ENV_SET_HW_RENDER = 14, ENV_SET_HW_SHARED_CONTEXT = 44, ENV_GET_VARIABLE = 15, ENV_SET_VARIABLES = 16,
...
/* Images entierement noires consecutives : sert a detecter qu'on a demande
   une finesse que l'appareil ne suit pas. */
static int g_noires = 0;

/* Une barriere par tampon : elle dit quand la copie demandee au processeur
   graphique est reellement terminee. Sans elle, on lisait un tampon encore en
   cours d'ecriture — a 640 x 480 la copie tenait dans une image et personne ne
   s'en apercevait, mais a 1920 x 1440 elle pese onze megaoctets et deborde
   largement : d'ou l'image dechiree des qu'on monte en finesse. */
/* Tampon de sortie : l'image reduite qu'on relit reellement.
 *
 * Le jeu est calcule en tres haute finesse, puis l'image est ramenee a cette
 * taille par la carte graphique elle-meme, en une operation qui moyenne les
 * pixels — c'est exactement ce qui lisse une image, et c'est gratuit puisque
 * la carte le fait de toute facon mieux et plus vite que nous.
 *
 * Deux gains : la relecture garde une taille constante et modeste, quelle que
 * soit la finesse choisie ; et le moyennage supprime l'escalier des contours,
 * ce qu'une simple montee en definition ne fait pas. */
static GLuint g_fbo_sortie = 0, g_tex_sortie = 0;
/* Largeur a laquelle l'image est ramenee avant relecture. Zero la relit
   telle quelle, ce qui coute cher des qu'on monte en finesse. */
 ...
 * memoire de la console a chaque image, les codes GameCube sont au format
...
/* ---------- son ----------
 *
 * Dolphin fait tourner l'emulation sur son propre fil — le journal le
...
static int16_t g_gachette[2] = {0, 0};   /* inutilise sur GameCube */
...
static bool environnement(unsigned cmd, void *data) {
    switch (cmd & 0xFFFF) {
        ...
        case ENV_SET_HW_SHARED_CONTEXT:
            /*
             * Contexte graphique PARTAGE.
             *
             * Dolphin dessine sur son propre fil et exige que le contexte
             * soit partageable. Sans cette reponse, il n'affiche qu'une seule
             * texture, ou rien du tout — c'est la premiere cause d'ecran noir
             * signalee par ses auteurs.
             */
            noter("contexte graphique partage : accorde");
            return true;
...
Java_com_skingc_app_core_CoeurGC_natInit(JNIEnv *env, jobject self, jstring chemin,
    ...
    /* Dolphin alloue sa memoire virtuelle dans un fichier temporaire. Sur
    ...
    /*
     * Reglages du coeur Dolphin.
     *
     * Les noms viennent de sa documentation, pas de ma memoire : un libelle
     * qu'il ne reconnait pas est ignore en silence, et l'on croit ensuite que
     * le reglage ne sert a rien.
     */
    var_poser("dolphin_renderer", "Hardware");
    var_poser("dolphin_efb_scale", "x1 (640 x 528)");
    var_poser("dolphin_shader_compilation_mode", "Synchronous (Ubershaders)");
    var_poser("dolphin_cheats_enabled", "enabled");   /* pour les codes de triche */
    var_poser("dolphin_widescreen", "disabled");
    var_poser("dolphin_progressive_scan", "enabled");
    var_poser("dolphin_language", "French");
    var_poser("dolphin_cpu_core", "JIT64");
    var_poser("dolphin_fastmem", "enabled");
    var_poser("dolphin_dsp_hle", "enabled");
    ...
    noter("initialisation de Dolphin...");
    p_init();
    noter("Dolphin initialise");
    installer_signaux();
    g_init = true;
    noter("coeur Dolphin pret, %d variables posees", g_nvars);
    return JNI_TRUE;
}

JNIEXPORT void JNICALL
Java_com_skingc_app_core_CoeurGC_natVariable(JNIEnv *env, jobject self, jstring cle, jstring val) {
    (void)self;
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_skingc_app_core_CoeurGC_natGlInit(JNIEnv *env, jobject self, jint w, jint h) {
    (void)env; (void)self;
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_skingc_app_core_CoeurGC_natGlTaille(JNIEnv *env, jobject self, jint w, jint h) {
    (void)env; (void)self;
...
Java_com_skingc_app_core_CoeurGC_natGlImage(JNIEnv *env, jobject self, jint boutons,
...
JNIEXPORT jboolean JNICALL
Java_com_skingc_app_core_CoeurGC_natCharger(JNIEnv *env, jobject self, jstring chemin) {
    (void)self;
     ...
     * chemin. Dolphin lit le disque depuis son fichier : il l'annonce
    ...
    /* Manette GameCube : un JOYPAD. Son unique stick est lu par les
     ...
     *
     * C'etait la cause de l'image agrandie : le tampon gardait la taille creee
     * au demarrage, 640 x 480, alors que le coeur dessinait en 1920 x 1440 des
     * qu'on montait la finesse. Il ne remplissait donc que le coin inferieur
     * gauche du tampon, et c'est ce coin qu'on affichait en plein ecran.
     */
    if (g_hw_demande) {
        ...
        if (vw != g_fw || vh != g_fh) {
            /* Repli progressif.
             *
             * Un tampon de 1920 x 1440 avec profondeur et pochoir demande une
             * quinzaine de megaoctets de memoire graphique, et toutes les
             * cartes de telephone ne l'accordent pas. Echouer la laissait un
             * ecran noir sans un mot. On redescend donc par moities jusqu'a
             * ce que ca passe, et on le dit. */
            bool pose = false;
            int tw = vw, th = vh;
    ...
    /* Dolphin ne declare son rendu materiel qu'au chargement du jeu : c'est
...
JNIEXPORT jint JNICALL
Java_com_skingc_app_core_CoeurGC_natLirePixels(JNIEnv *env, jobject self, jintArray sortie) {
    (void)self;
    if (!g_charge) return 0;
    ...
    if (!g_hw_demande) {
        ...
        pthread_mutex_unlock(&g_image_verrou);
        if (g_mesures < 8) { g_mesures++; noter("image recue : %u x %u", lw, lh); }
    ...
    /* Reduction sur la carte graphique.
     *
     * Le jeu est calcule en haute finesse, mais on ne relit qu'une image
     * ramenee a une taille raisonnable. La carte fait cette reduction en
     * moyennant les pixels, ce qui adoucit les contours bien mieux qu'une
     * simple montee en definition — et la relecture garde un cout constant,
     * quelle que soit la finesse choisie. */
    unsigned sw = w, sh = h;
    ...
    if ((jsize)(pw * ph) > cap) {
        /* Cas silencieux qui laissait l'ecran noir : l'image relue depassait
           le tableau et on abandonnait sans un mot. */
        noter("ECHEC : image %u x %u (%u pixels) trop grande pour le tableau (%d)",
              pw, ph, pw * ph, (int)cap);
    ...
    if (somme == 0) g_noires++; else g_noires = 0;
    if (g_mesures < 8) {
        g_mesures++;
        noter("image recue : %u x %u (soit %.1f fois la definition d'origine)",
              pw, ph, (double)pw / 320.0);
    }
...
/* Jette les echantillons en attente. Apres une reinitialisation ou une
   relance, ils appartiennent a la partie precedente : les jouer donne ce
   grondement qu'on entend au redemarrage. */
/* Largeur a laquelle l'image est ramenee avant relecture. Zero la relit telle
   quelle, ce qui coute cher des qu'on monte en finesse. */
/* Nombre d'images noires d'affilee. Au-dela de quelques dizaines, la finesse
   demandee ne passe pas sur cet appareil. */
JNIEXPORT jint JNICALL
Java_com_skingc_app_core_CoeurGC_natNoires(JNIEnv *e, jobject s) {
    (void)e; (void)s; return (jint)g_noires;
}

JNIEXPORT void JNICALL
Java_com_skingc_app_core_CoeurGC_natReduction(JNIEnv *e, jobject s, jint largeur) {
    (void)e; (void)s;
    g_reduction = largeur;
...
JNIEXPORT void JNICALL
Java_com_skingc_app_core_CoeurGC_natSonVider(JNIEnv *e, jobject s) {
    (void)e; (void)s;
    pthread_mutex_lock(&g_son_verrou);
    g_son_n = 0;
    pthread_mutex_unlock(&g_son_verrou);
...
JNIEXPORT jint JNICALL
Java_com_skingc_app_core_CoeurGC_natSon(JNIEnv *env, jobject self, jshortArray sortie) {
    (void)self;
...
JNIEXPORT jboolean JNICALL
Java_com_skingc_app_core_CoeurGC_natRenduMateriel(JNIEnv *e, jobject s) {
    (void)e; (void)s; return g_hw_demande ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jint JNICALL
Java_com_skingc_app_core_CoeurGC_natMaxL(JNIEnv *e, jobject s) { (void)e; (void)s; return (jint)g_maxw; }
JNIEXPORT jint JNICALL
Java_com_skingc_app_core_CoeurGC_natMaxH(JNIEnv *e, jobject s) { (void)e; (void)s; return (jint)g_maxh; }

JNIEXPORT jdouble JNICALL Java_com_skingc_app_core_CoeurGC_natFrequence(JNIEnv *e, jobject s) { (void)e; (void)s; return g_sample_rate; }
JNIEXPORT jdouble JNICALL Java_com_skingc_app_core_CoeurGC_natFps(JNIEnv *e, jobject s) { (void)e; (void)s; return g_fps; }
JNIEXPORT void JNICALL Java_com_skingc_app_core_CoeurGC_natReset(JNIEnv *e, jobject s) { (void)e; (void)s; if (g_charge) p_reset(); }

JNIEXPORT void JNICALL
Java_com_skingc_app_core_CoeurGC_natEjecter(JNIEnv *env, jobject self) {
    (void)env; (void)self;
    if (g_charge) {
        /* On annonce d'abord au coeur que son contexte graphique disparait.
         *
         * Sans cette annonce, il gardait toutes ses ressources — textures,
         * shaders, tampons — et le chargement suivant lui en faisait creer un
         * second jeu par-dessus le premier. D'ou l'image qui se brouille et le
         * son qui deraille apres un changement de finesse. C'est la meme faute
         * que le signal envoye trois fois, sous une autre forme : ici il
         * manquait. */
...
JNIEXPORT jboolean JNICALL
Java_com_skingc_app_core_CoeurGC_natTricheDispo(JNIEnv *e, jobject s) {
    (void)e; (void)s;
    return (p_cheat_set && p_cheat_reset) ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT void JNICALL
Java_com_skingc_app_core_CoeurGC_natTricheVider(JNIEnv *e, jobject s) {
    (void)e; (void)s;
    if (p_cheat_reset) p_cheat_reset();
}

/* Un code GameShark, transmis tel quel au coeur. Les lignes multiples sont
   separees par un plus dans la base ; le coeur sait les lire. */
JNIEXPORT void JNICALL
Java_com_skingc_app_core_CoeurGC_natTriche(JNIEnv *env, jobject self, jint index,
                                             jboolean actif, jstring code) {
    (void)self;
    if (!p_cheat_set) return;
    const char *c = (*env)->GetStringUTFChars(env, code, NULL);
    p_cheat_set((unsigned)index, actif == JNI_TRUE, c);
    (*env)->ReleaseStringUTFChars(env, code, c);
}

JNIEXPORT jbyteArray JNICALL
Java_com_skingc_app_core_CoeurGC_natSauver(JNIEnv *env, jobject self) {
    (void)self;
...
JNIEXPORT jboolean JNICALL
Java_com_skingc_app_core_CoeurGC_natRestaurer(JNIEnv *env, jobject self, jbyteArray donnees) {
    (void)self;
